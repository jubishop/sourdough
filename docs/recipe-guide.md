...
On 2026-09-18, the user changed the autolyse to 30 minutes to 2 hours,
revising their initial 1–2-hour range earlier that day. The reason was not
stated. This supersedes the fixed one-hour rest without restoring the
earlier two-hour target. Start when the starter looks about 30 minutes to
2 hours from ready. Keep the step, schedule, and overlap diagram consistent
with this range.
...
last stretch and fold. Leave the dough undisturbed for the rest of bulk
...
minutes were a trial to allow more baking and moisture loss, with a darker,
firmer crust as the tradeoff. This superseded the earlier 20-minute uncovered
setting while retaining the 30-minute covered bake at 450°F and the existing
fermentation and cooling instructions.

On 2026-09-14, the user chose to try 21 minutes uncovered at 425°F because
...
On 2026-09-24, the user extended the uncovered bake to 25 minutes at the
435°F oven setting. This supersedes the 24-minute setting and retains the
fixed endpoint. The reason was not stated.

Later on 2026-09-24, the user lowered the uncovered oven setting by another
5°F to 430°F and extended the time by one minute to 26 minutes. This
supersedes 25 minutes at a 435°F oven setting and retains the fixed
endpoint. The reason was not stated.

Later that day, the user restored the uncovered oven setting to 435°F.
...
The user then simplified the lining to one flat layer of foil with no greasing
because greasing was too much work. At that time, this replaced the thick foil
pad, second sheet up the sides, and oiling instructions. The 2026-09-12 pad
decision below supersedes the single flat layer.
...
the first fold in a session shows stretch marks. They clarified that the
...
Each wait starts after the previous session ends. The user initially
allowed up to two stretch-and-folds per session, then specified two in
the first session and one in each later session. They also simplified
the stopping cue to "Stop if you see any stretch marks", replacing the
condition about the first stretch-and-fold in a session.
The reason was not stated. This supersedes the fixed
